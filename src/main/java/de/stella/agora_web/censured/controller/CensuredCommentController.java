...
public class CensuredCommentController {
    ...
    public ResponseEntity<Boolean> isCommentCensured(@PathVariable Long id) {
        Optional<CensuredComment> censuredComment = censuredCommentRepository.findById(id);
        return censuredComment.isPresent() ? ResponseEntity.ok(true) : ResponseEntity.ok(false);
    }

    @GetMapping("/user/{userId}")
    public ResponseEntity<Integer> countUserInfractions(@PathVariable Long userId) {
        List<CensuredComment> censuredComments = censuredCommentRepository.findByUserId(userId);
        return ResponseEntity.ok(censuredComments.size());
    }

    @PostMapping("/censure")
    public ResponseEntity<Void> censureComment(@RequestBody CensuredCommentDTO censuredCommentDTO) {
        Optional<Comment> commentOptional = Optional.of(commentService.findById(censuredCommentDTO.getCommentId()));
        if (commentOptional.isPresent()) {
            Comment comment = commentOptional.get();
            User user = comment.getUser();
            String reason = censuredCommentDTO.getReason();
            CensuredComment censuredComment = new CensuredComment(comment.getId(), user, reason);
